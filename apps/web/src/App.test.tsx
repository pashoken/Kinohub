...
describe("catalog surfaces", () => {
  ...
  it("recommends unrated titles with genres preferred by the viewer", () => {
    ...
    const recommendations = buildRecommendations(candidates, { [`movie:${liked.id}`]: { value: 8, item: liked } });
  ...
  it("places recommendations third after popular titles and popular series", () => {
    ...
    render(<Home catalog={fixtureCatalog} series={[show]} tasteProfile={{ [`movie:${liked.id}`]: { value: 8, item: liked } }} />);
  ...
  it("persists a 1–10 score and immediately shows recommendations on home", async () => {
    const movie = fixtureCatalog.rails[0]!.movies[0]!;
...
describe("torrent choice drawer", () => {
  ...
  it("shows a generic elapsed-time status while trackers respond", () => {
